  ...
  //---------------------------------------------------MAKING THE QUEUE-------------------------------------------------------
  // Code from "Queue Data Structure implementation using Array - C++" on by CodeWhoop on Youtube
  #include <iostream>
  using namespace std;
  #define SIZE 5
  int A[SIZE};
  ...
  //Description : Used as the feedback system when an emotion button is pressed, this twinkling
  //effect is what is sent to the lights. The emotionNum parameter is used to figure what color
  //the lights should twinkle in. Alternating groups 3 & 4 are used. The current state of the
  //twinkle effect utilizes the changeGroup method to alternate the saturations of the hue
  //between both groups.
  void twinkle(int emotionNum) {
    String hueString = "";
    if(emotionNum = 1) { // happy emotion, color is yellow
      hueString = "11000"; 
    } else if (emotionNum = 2){ // neutral emotion, color is green
      hueString = "20000";
    } else (emotionNum = 3){ // sad emotion, color is blue
      hueString = "42000";
    }
    changeGroup(0, 1, "on", "true", "hue", "42000", "bri", "254", "sat", "100"); //set all of the lights to a cool white
    changeGroup(3, 6, "on", "true", "hue", hueString, "bri", "254", "sat", "250");
    changeGroup(4, 6, "on", "true", "hue", hueString, "bri", "254", "sat", "150");
    changeGroup(3, 6, "on", "true", "hue", hueString, "bri", "254", "sat", "150");
    changeGroup(4, 6, "on", "true", "hue", hueString, "bri", "254", "sat", "250");
  }
  ...
  //Description: Checks if there are ties among the 3 button counts. Returns true if and only
  // if there is a tie.
  boolean tiesExist() {
    if(countButton1 == countButton2 || countButton1 == countButton3 || countButton2 == countButton3) {
      return true;
    }
    return false;
    }

  //Description : THIS IS MEANT TO BE USED ALONG WITH THE tiesExist() METHOD,. If all three counts are tied,
  //greatestEmotion returns all three emotion numbers. If there is only one tie among the three button counts,
  //then return the emotion number(s) with the greatest count. This implies that if
  //the tieExist() returns true, then the two emotion numbers not returned are tied.
  //USE THIS WHEN TIESEXIST IS TRUE.
  int greatestEmotion() {
    if(countButton1 == countButton2 && countButton1 == countButton3) {
      return 321;
    } else if (countButton1 == countButton2 && countButton3 > countButton1) {
      return 3;
    } else if (countButton1 == countButton2 && countButton3 < countButton1) {
      return 21;
    } else if (countButton1 == countButton3 && countButton2 > countButton1) {
      return 2;
    } else if (countButton1 == countButton3 && countButton2 < countButton1) {
      return 31;
    } else if (countButton2 == countButton3 && countButton1 > countButton3) {
      return 1;
    } else if (countButton2 == countButton3 && countButton1 < countButton3) {
      return 32;
    }
  ...
  //Description : THIS IS MEANT TO BE USED ALONG WITH THE tiesExist() METHOD. If tiesExist() is false, then use this
  //method to determine the greatest to least order of the emotions.
  int rank() {
    // possible combos : 123, 132, 213, 231, 312, 321
    if(countButton1 < countButton2 && countButton2 < countButton3) {
      return 321;
    } else if(countButton1 < countButton3 && countButton3 < countButton2) {
      return 231;
    } else if(countButton2 < countButton1 && countButton1 < countButton3) {
      return 312;
    } else if(countButton2 < countButton3 && countButton3 < countButton1) {
      return 132;
    } else if(countButton3 < countButton2 && countButton2 < countButton1) {
      return 123;
    } else {
      return 213;
    }
  ...
  //Description: Each hour the summary visualization is displayed. The visualization is a comparison
  //of all three emotions as a bar graph when looking up.
  void summaryViz() {
    int viz = 0; // Variable to hold which color palette we will visualize.
    if(tiesExist()) {
      viz = greatestEmotion();
      if(viz == 321) {
        sendPalette("M_SUMMARY_T_321");
      } else if (viz == 3) {
        sendPalette("M_SUMMARY_T_3");
      } else if (viz == 21) {
        sendPalette("M_SUMMARY_T_21");
      } else if (viz == 2) {
        sendPalette("M_SUMMARY_T_2");
      } else if (viz == 31) {
        sendPalette("M_SUMMARY_T_31");
      } else if (viz == 1) {
        sendPalette("M_SUMMARY_T_1");
      } else {
        sendPalette("M_SUMMARY_T_32"); // for 32
      }
    } else {
      viz = rank();
      if(viz = 321) {
        sendPalette("M_SUMMARY_F_321");
      } else if (viz == 213) {
        sendPalette("M_SUMMARY_F_213");
      } else if (viz == 312) {
        sendPalette("M_SUMMARY_F_312");
      } else if (viz == 132) {
        sendPalette("M_SUMMARY_F_132");
      } else if (viz == 123) {
        sendPalette("M_SUMMARY_F_123");
      } else {
        sendPalette("M_SUMMARY_F_213"); // for 213
      }
    }
  }
